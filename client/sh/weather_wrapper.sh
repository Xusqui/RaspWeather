...
if [ -z "$SERVER_URL" ]; then
  echo "Specify the URL of the 'client' directory on your server, e.g. http://server.com/client"
  echo "If authentication is required, use http://user:pass@server.com/client"
  exit 1
fi

# Internal variables.
GREETING="WeatherPi wrapper 0.1.0 - (c) 2017 Francisco Fernandez - http://fernandezvillatoro.es Based upon job of Joerg Zieren - http://zieren.de - GNU GPL v3"
DL_FILENAME=weather-client.zip
...
while true; do
  ...
  if [ $UPDATE_CLIENT -ne 0 ]; then
    ...
    prune $NUM_DIRS_TO_KEEP $(ls_weather_dirs)
  fi
  log "starting client"
  cd current
