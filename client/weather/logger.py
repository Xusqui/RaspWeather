...
class LoggerFactory:
  ...
  _LOG_DIR = '../logs/'
  _LOG_EXT = '.log'
  _LOG_FORMATS = {'weather': '%(asctime)s - %(name)-10s - %(levelname)-8s - %(message)s',
  ...
  def _get_toplevel_logger(self, name):
    """Return a toplevel logger (e.g. 'weather'), which propagates its configuration to lower-level
    loggers (e.g. 'weather.wind') by means of the logging framework. Create and configure the logger if
